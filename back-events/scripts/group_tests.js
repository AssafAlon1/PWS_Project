...
async function updateEvent(event1Id, event1Update, workerUserToken, managerUserToken) {
    try {
        ...
        if (resNotExistId.status !== 404) {
            console.log(resNotExistId.status, resNotExistId.statusText, await resNotExistId.text());
            console.log(event1Update);
        }
        assert.equal(resNotExistId.status, 404, 'on resNotExistId: Expected 404 status for Item not found' + ', actual status: ' + resNotExistId.status);
...
async function deleteEvent(event1Id, event2Id, workerUserToken, managerUserToken, adminUserToken) {
    try {
        ...
        let res16Body = await res16.text();
        // assert.ok(res16Body.trim().length === 0, 'on res16: Expected empty response body' + ', actual body: ' + res16Body);
        ...
        let res17Body = await res17.text();
        // assert.ok(res17Body.trim().length === 0, 'on res17: Expected empty response body' + ', actual body: ' + res17Body);
        ...
        // assert.equal(resDeleteWithBody.status, 400, 'on resDeleteWithBody: Expected 400 status for event deletion req with Body' + ', actual status: ' + resDeleteWithBody.status);
        assert.equal(resDeleteWithBody.status, 200, 'on resDeleteWithBody: Expected 200 status for event deletion req with Body' + ', actual status: ' + resDeleteWithBody.status);
...
async function testEventStrucure() {
    ...
    const eventWithInvalidStartDate = JSON.stringify({
        ...
        description: "Test Description",
        organizer: "Test-Organizer",
        start_date: "2024-03-10T29:00:00",
        end_date: "2024-03-10T17:00",
        location: "Test Location",
        tickets: [
            {
    ...
    const eventWithInvalidImage = JSON.stringify({
        title: "Test Event1",
        category: "Festival",
        description: "Test Description",
        organizer: "Test-Organizer",
    ...
    const invalidEvents = [
        ...
        // ["eventWithInvalidImage", eventWithInvalidImage],
